...
Options are `--seed <whole number>`, `--pack <positive whole number>`, repeatable `--avoid <exact destination name>` (case-insensitive), `--format text|html`, `--save <path>`, and `--self-test`. Text is the default; HTML is a standalone printable postcard with inline CSS and no external assets. `--help` lists valid destination names. Unknown exclusions and exclusions that leave fewer than four available destinations are rejected. No packages are required; the program uses Swift standard library and Foundation only. The self-test runs the real generator and both renderers to check same-seed determinism, different-seed variation, packing bounds, HTML escaping, Unicode option rejection, and invalid option handling.
