...
Options are `--seed <whole number>`, `--pack <positive whole number>`, `--save <path>`, and `--self-test`. No packages are required; the program uses Swift standard library and Foundation only. The self-test runs the real generator and renderer to check same-seed determinism, different-seed variation, packing bounds, Unicode option rejection, and invalid option handling.
